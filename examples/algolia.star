# Algolia docs extraction/enrichment script.
...
CONTENT_SELECTOR = HEADING_SELECTOR + ", span[data-as=p], li, " + PARAM_FIELD_SELECTOR
...
# Called before records(). Returns page-level metadata from stable docs selectors.
def page_meta(doc, ctx):
    return {
        "title": clean_text(doc.select_one("h1#page-title")),
        "description": clean_attr(doc.select_one("meta[name=description]"), "content"),
    }


# Main extraction pass.
# Emits page record first, then walks content nodes in DOM order.
def records(doc, ctx):
    meta = ctx["metadata"]["pageMeta"]
    page = page_record(ctx["url"], meta)
    out = [page]
...
def enrich(record, ctx):
    # Records are fully shaped in records() to preserve legacy ordering/state.
    return record


# New and older docs layouts use different content container IDs.
def content_root(doc):
    for selector in ["#content", "#content-area"]:
...
def page_record(page_url, meta):
    ...
    content_type = content_type_from_url(base_url)
    if content_type != "":
        record["contentType"] = content_type
    variant = variant_from_url(base_url)
    if variant != "":
        record["variant"] = variant
    method = method_name_from_url(base_url)
    if method != "":
...
def field_record(page, current_hierarchy, page_url, node, position):
    ...
    if anchor == "":
        return None
    name = clean_text(node.select_one("[data-component-part=field-name]"))
    if name == "":
        return None
    hierarchy = clone_hierarchy(current_hierarchy)
...
def field_description(node):
    parts = []
    pill = clean_text(node.select_one("[data-component-part=field-info-pill]"))
    if pill != "":
        parts.append(pill)
    required = clean_text(node.select_one("[data-component-part=field-required-pill]"))
    if required != "":
...
# Normalize extracted text to match historical JSONL output.
def clean_text(node):
    if node == None:
        return ""
...
# Derive camelCase methodName only for REST API and SDK method pages.
def method_name_from_url(page_url):
    value = breadcrumb_path(page_url).strip("/")
    if not value.startswith("rest-api/") and not value.startswith("libraries/sdk/methods/"):
        return ""
    parts = value.split("/")
    if len(parts) == 0:
        return ""
...
# Coarse content type used as a facet/filter in the search index.
def content_type_from_url(page_url):
    value = breadcrumb_path(page_url)
    if value.startswith("/guides"):
        return "guide"
    if value.startswith("/rest-api"):
        return "api"
    if value.startswith("/api-reference/api-parameters"):
        return "api"
    if value.startswith("/integration"):
        return "integration"
    if value.startswith("/libraries/sdk"):
        return "sdk"
    if value.startswith("/framework-integration"):
        return "sdk"
    return ""


# Mark legacy doc variants where URL convention identifies version.
def variant_from_url(page_url):
    value = breadcrumb_path(page_url)
    if value.startswith("/libraries/sdk/v1"):
        return "legacy"
    if value.startswith("/rest-api/ingestion/"):
        last = value.split("/")[-1]
        if last.endswith("v1"):
            return "legacy"
    return ""
